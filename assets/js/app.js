// ================= 鏁版嵁鍒濆鍖栦笌宸ュ叿鍑芥暟 =================
        ...
        const getTimeStrFromDate = d => `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
        const getLogicalDateStrFromDate = date => {
        ...
        function normalizeDataStore(raw = {}) {
            ...
            Object.keys(normalized.todoData).forEach(dateStr => {
                normalized.todoData[dateStr] = normalized.todoData[dateStr].map(item => ({
                    ...
                    longPlanChildId: typeof item?.longPlanChildId === 'string' ? item.longPlanChildId : '',
        ...
        function getMonthlySettlement(dateStr = getTodayStr()) {
            ...
            const offsetPoints = Math.floor(bonusCount / 2);
        ...
        function getMonthlySavingSettlements(limit = 12) {
            const monthKeys = new Set([getMonthKey()]);
            [
                D.penaltyData?.records,
                D.bonusData?.records,
                D.checkinData,
                D.taskData
            ].forEach(source => {
                Object.keys(source || {}).forEach(dateStr => {
                    const monthKey = getMonthKey(dateStr);
            ...
            return Array.from(monthKeys)
                ...
                .slice(0, limit)
                .map(monthKey => getMonthlySettlement(monthKey));
        }
        ...
        function isWakeOk(timeStr) {
            const minutes = parseTimeToMinutes(timeStr);
            return minutes !== null && minutes < 9 * 60;
        }

        function isSleepOk(timeStr) {
            const minutes = parseTimeToMinutes(timeStr);
            return minutes !== null && minutes >= 18 * 60 && minutes <= 23 * 60 + 30;
        }

        function getRequiredTimeInput(id) {
            const value = document.getElementById(id)?.value;
            if (!value || parseTimeToMinutes(value) === null) {
                alert('请先选择有效的打卡时间');
                return null;
            }
            return value;
        ...
        function finishCheckinWrite(dateStr) {
            syncAutoPenalties(dateStr);
            saveData();
            updateCheckinUI();
        }

        function recordWake(dateStr, timeStr) {
            ensureDateDataExists(dateStr);
            D.sleepData[dateStr].wake = timeStr;
            D.sleepData[dateStr].wakeOk = isWakeOk(timeStr);
            finishCheckinWrite(dateStr);
        }

        function recordSleep(dateStr, timeStr) {
            ensureDateDataExists(dateStr);
            D.sleepData[dateStr].sleep = timeStr;
            D.sleepData[dateStr].sleepOk = isSleepOk(timeStr);
            finishCheckinWrite(dateStr);
        }

        function recordWorkTime(dateStr, field, timeStr) {
            ensureDateDataExists(dateStr);
            D.checkinData[dateStr].work[field] = timeStr;
            D.checkinData[dateStr].work.status[field] = field === 'checkIn' ? isWorkCheckInOk(timeStr) : isWorkCheckOutOk(timeStr);
            finishCheckinWrite(dateStr);
        }
        ...
        function carryForwardUnfinishedTodos(targetDateStr = getTodayStr()) {
            ...
            latestTodoByLineage.forEach(({ todo, dateStr }, lineageId) => {
                ...
                todayTodos.push({
                    ...
                    planId: todo.planId || '',
                    longPlanChildId: todo.longPlanChildId || '',
                    lineageId,
                    carriedFrom: dateStr
                });
        ...
        function initCheckin() {
            const t = getTodayStr();
            ensureDateDataExists(t);
            document.getElementById('btn-wake').onclick = () => recordWake(getTodayStr(), getTimeStr());
            document.getElementById('btn-wake-custom').onclick = () => {
                const timeStr = getRequiredTimeInput('wake-custom-time');
                if (timeStr) recordWake(getTodayStr(), timeStr);
            };
            document.getElementById('btn-wake-yesterday').onclick = () => {
                const timeStr = getRequiredTimeInput('wake-custom-time');
                if (timeStr) recordWake(getTodayStr(-1), timeStr);
            };
            document.getElementById('btn-sleep').onclick = () => {
                const delayMinutes = document.getElementById('sleep-delay-30')?.checked ? 30 : 0;
                const sleepAt = new Date(Date.now() + delayMinutes * 60 * 1000);
                recordSleep(getLogicalDateStrFromDate(sleepAt), getTimeStrFromDate(sleepAt));
            };
            document.getElementById('btn-sleep-custom').onclick = () => {
                const timeStr = getRequiredTimeInput('sleep-custom-time');
                if (timeStr) recordSleep(getTodayStr(), timeStr);
            };
            document.getElementById('btn-sleep-yesterday').onclick = () => {
                const timeStr = getRequiredTimeInput('sleep-custom-time');
                if (timeStr) recordSleep(getTodayStr(-1), timeStr);
            };
            document.getElementById('work-checkin').onclick = () => {
                recordWorkTime(getTodayStr(), 'checkIn', getTimeStr());
            };
            document.getElementById('work-checkin-custom').onclick = () => {
                const timeStr = getRequiredTimeInput('work-checkin-custom-time');
                if (timeStr) recordWorkTime(getTodayStr(), 'checkIn', timeStr);
            };
            document.getElementById('work-checkin-yesterday').onclick = () => {
                const timeStr = getRequiredTimeInput('work-checkin-custom-time');
                if (timeStr) recordWorkTime(getTodayStr(-1), 'checkIn', timeStr);
            };
            document.getElementById('work-checkout').onclick = () => {
                recordWorkTime(getTodayStr(), 'checkOut', getTimeStr());
            };
            document.getElementById('work-checkout-custom').onclick = () => {
                const timeStr = getRequiredTimeInput('work-checkout-custom-time');
                if (timeStr) recordWorkTime(getTodayStr(), 'checkOut', timeStr);
            };
            document.getElementById('work-checkout-yesterday').onclick = () => {
                const timeStr = getRequiredTimeInput('work-checkout-custom-time');
                if (timeStr) recordWorkTime(getTodayStr(-1), 'checkOut', timeStr);
            };
        ...
        function getLongPlanChildTodoLineageId(childId) {
            return `lpc_todo_${childId}`;
        }

        function getLongPlanChild(planId, childId) {
            const plan = getLongPlanById(planId);
            if (!plan) return { plan: null, child: null };
            const child = plan.children.find(item => item.id === childId) || null;
            return { plan, child };
        }

        function createTodoFromLongPlanChild(plan, child, targetDateStr = getTodayStr(), extra = {}) {
            ensureDateDataExists(targetDateStr);
            return {
                ...
                planId: plan.id,
                longPlanChildId: child.id,
                lineageId: getLongPlanChildTodoLineageId(child.id),
                carriedFrom: extra.carriedFrom || ''
            };
        }

        function ensureLongPlanChildrenInTodos(targetDateStr = getTodayStr()) {
            ensureDateDataExists(targetDateStr);
            const todayTodos = D.todoData[targetDateStr];
            const todayChildIds = new Set(todayTodos.map(todo => todo.longPlanChildId).filter(Boolean));
            let changed = false;
        ...
        function syncTodoStateFromLongPlanChild(planId, childId, done) {
            const { plan, child } = getLongPlanChild(planId, childId);
            if (!plan || !child) return false;

            const todayStr = getTodayStr();
            ensureDateDataExists(todayStr);
            let changed = false;
            let latestTodoRef = null;

            Object.keys(D.todoData || {})
                .sort()
                .forEach(dateStr => {
                    (D.todoData[dateStr] || []).forEach(todo => {
                        if (todo.longPlanChildId === childId) {
                            latestTodoRef = todo;
                            if (dateStr === todayStr && todo.done !== done) {
                                todo.done = done;
                                changed = true;
                            }
                        }
            ...
            if (latestTodoRef && latestTodoRef.done !== done) {
                latestTodoRef.done = done;
                changed = true;
            }

            if (!done) {
                const todayTodo = (D.todoData[todayStr] || []).find(todo => todo.longPlanChildId === childId);
                if (!todayTodo) {
                    D.todoData[todayStr].push(createTodoFromLongPlanChild(plan, child, todayStr));
                    changed = true;
                }
            }
        ...
        function renderTodos() { 
            ...
            const todayStr = getTodayStr();
            let changed = false;
            if (carryForwardUnfinishedTodos(todayStr)) changed = true;
            if (ensureLongPlanChildrenInTodos(todayStr)) changed = true;
            if (changed) saveData({ touchLocalMeta: false });
            container.innerHTML = ''; 
            const todayTodos = D.todoData[todayStr] || [];
        ...
        window.toggleTodo = function(id) {
            ...
            if (!todo) return;
            todo.done = !todo.done;
            if (todo.longPlanChildId && todo.planId) {
                const { child } = getLongPlanChild(todo.planId, todo.longPlanChildId);
                if (child) child.done = todo.done;
            }
            saveData();
            renderLongPlans();
            renderTodos();
        ...
        window.deleteLongPlan = function(planId) {
            ...
            Object.keys(D.todoData).forEach(dateStr => {
                D.todoData[dateStr] = (D.todoData[dateStr] || []).map(todo => todo.planId === planId ? { ...todo, planId: '', longPlanChildId: '' } : todo);
            });
            saveData();
            renderLongPlans();
            renderTodos();
        ...
        window.addLongPlanChild = function(planId) {
            ...
            const plan = getLongPlanById(planId);
            if (!plan) return;
            const child = { id: 'lpc_' + Date.now(), name, done: false };
            plan.children.unshift(child);
            ensureDateDataExists(getTodayStr());
            D.todoData[getTodayStr()].push(createTodoFromLongPlanChild(plan, child));
            input.value = '';
            saveData();
            renderTodos();
            renderLongPlans();
        ...
        window.toggleLongPlanChild = function(planId, childId) {
            const { child } = getLongPlanChild(planId, childId);
            if (!child) return;
            child.done = !child.done;
            syncTodoStateFromLongPlanChild(planId, childId, child.done);
            saveData();
            renderLongPlans();
            renderTodos();
        };
        ...
        window.deleteLongPlanChild = function(planId, childId) {
            ...
            Object.keys(D.todoData).forEach(dateStr => {
                D.todoData[dateStr] = (D.todoData[dateStr] || []).map(todo => todo.longPlanChildId === childId ? { ...todo, longPlanChildId: '', planId: '' } : todo);
            });
            saveData();
            renderLongPlans();
            renderTodos();
        ...
        function initHabits() {
            ...
            document.getElementById('btn-habit-yesterday').addEventListener('click', () => {
                dp.value = getTodayStr(-1);
        ...
        function renderPenaltySummary() {
            ...
            renderMonthlySavingOverview();
            renderStatsDateOverview();
        }

        function renderMonthlySavingOverview() {
            const list = document.getElementById('monthly-saving-list');
            if (!list) return;
            const settlements = getMonthlySavingSettlements();
            list.innerHTML = '';
            settlements.forEach(item => {
                const monthLabel = `${item.monthKey.slice(0, 4)}年${Number(item.monthKey.slice(5, 7))}月`;
                const savingClass = item.saving > 0 ? 'text-amber-600' : 'text-gray-400';
                list.innerHTML += `
                    <tr>
                        <td class="px-4 py-3 font-black text-textmain whitespace-nowrap">${monthLabel}</td>
                        <td class="px-4 py-3 text-right font-bold text-danger">${item.penaltyPoints}</td>
                        <td class="px-4 py-3 text-right font-bold text-warning">${item.bonusCount}</td>
                        <td class="px-4 py-3 text-right font-bold text-textmain">${item.netPoints}</td>
                        <td class="px-4 py-3 text-right font-black ${savingClass}">${item.saving} 元</td>
                    </tr>`;
            });
            if (!list.innerHTML) {
                list.innerHTML = '<tr><td colspan="5" class="px-4 py-4 text-center text-gray-400 text-sm">暂无月度结算数据</td></tr>';
            }
        }
